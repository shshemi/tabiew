...
impl<A> FromIterator<A> for RaggedVec<A> {
    fn from_iter<T: IntoIterator<Item = A>>(iter: T) -> Self {
        let mut jv = RaggedVec::default();
        jv.push(iter);
        jv
    }
}

impl<A> IntoIterator for RaggedVec<A> {
    type Item = A;
...
impl<T> Default for RaggedVec<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_new_creates_empty_jagged_vec() {
        let jv: RaggedVec<i32> = RaggedVec::new();
